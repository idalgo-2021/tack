...
**Tack** решает проблему файлового хаоса на телефоне: фото, видео, аудио и документы оказываются размазаны по галерее, диктофону и файловому менеджеру — связать их с контекстом и отправить вместе невозможно.

* Ключевой сценарий: создаёте заметку («Идея для приложения»), тут же прикрепляете фото, диктуете голосовое пояснение и прикладываете PDF. Через неделю находите по тегу и отправляете коллеге — Tack упаковывает заметку со всеми вложениями одним нажатием. Каждая заметка — готовый к отправке пакет, а не запись в текстовом файле.
...
* Добавить импорт заметок
* Доработать ползунок прослушивания аудио(сделать более функциональным)
